...
- 积分数字会在 OCR 定位到的余额同行中避开刷新图标后单独放大，并在文字右侧和底部保留随字号变化的上下文边距，防止过紧裁剪改变首位数字。首次漏读刷新图标时会在同一次领取尝试内最多读取 3 帧，不会立即消耗下一次领取机会。Buddy 入口兼容本机 Windows OCR 已观察到的 `8uddy加油站` 首字母误读。
...
- `build.ps1` 同时生成 `artifacts\WorkBuddyAutoClaim-v1.1.4.zip`；包内不包含本机 `config.json`、PDB、状态或诊断数据。
