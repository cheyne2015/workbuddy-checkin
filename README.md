...
3. 先识别“积分余额”并记录其数字指纹，同时确认“设置 / 外观 / 浅色 / 深色”等个人中心菜单文字；未同时确认时拒绝点击，避免把其他页面的余额误当作个人中心。
4. 在余额附近的同一张个人中心卡片内，动态寻找 `签到领积分`、`立即领取`、`去签到`、`签到`、`领积分`、`领取`、`体验` 等文字。每个识别到的候选只点击一次。
5. 每点一次立即重新 OCR，比较本次点击前后的积分余额。余额数字变化即为领取成功；也会识别“今日已领”“本期已领”“领取成功”“签到成功”。`+100` 仅在点击之后作为辅助奖励证据，绝不会在点击前把可领取按钮误判为已领。
...
- `ClaimActionKeywords`：可领取按钮的 OCR 关键词；遇到版本更新时在此增加新文案即可，不要删除现有词。`ClaimActionExclusions` 用于排除同区域内不是领取动作的相似文案（默认排除“体验版”）。
- `BalanceValueVerticalTolerance` 与 `ClaimAction*Pixels`：余额数值和领取文字相对余额锚点的容差，已经为不同界面做了较宽的默认值；若 WorkBuddy 改版布局，再按实际截图微调。
